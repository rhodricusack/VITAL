...
class LRPModelRestricted(nn.Module):
    """Class wraps PyTorch model to perform layer-wise relevance propagation."""
    ...
    def forward(self, x: torch.tensor, topk=-1, channel=None, return_nodes=None) -> torch.tensor:
        ...
        if channel is None:
            ...
            channel_vector = torch.zeros(relevance.shape[1]).to(relevance.device)
            channel_vector[channel] = 1.0
            
            cos_sim = torch.nn.CosineSimilarity(dim=1)(channel_vector[None,:,None,None],relevance)
            max_vals_row, col_indices = torch.max(cos_sim, dim=2)

            _, row_indices = torch.max(max_vals_row, dim=1)

            final_col_indices = col_indices[torch.arange(cos_sim.shape[0]), row_indices]
            batch_indices = torch.arange(cos_sim.shape[0])

            channel_vector = channel_vector.repeat(relevance.shape[0],1)

            relevance_zero[batch_indices, :, row_indices, final_col_indices] = channel_vector
            relevance = relevance_zero
