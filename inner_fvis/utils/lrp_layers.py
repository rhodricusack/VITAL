...
from utils.lrp_filter import relevance_filter
...
class RelevancePropagationAvgPool2d(nn.Module):
    ...
    def __init__(
        self, layer: torch.nn.AvgPool2d, eps: float = 1.0e-12, top_k: float = 0.0
    ) -> None:
        super().__init__()
        self.layer = layer
        self.eps = eps
...
class RelevancePropagationDenseLayer(nn.Module):
    def __init__(self, layer, eps=1.0e-05, top_k=0.0):
        """
        Layer-wise relevance propagation for _DenseLayer in DenseNet.
        Attributes:
            layer: The _DenseLayer to be handled.
            eps: Small value for numerical stability.
            top_k: Optional filter to keep top_k% of relevance scores.
        """
        super().__init__()
        self.layer = layer
        self.layers = [
    ...
    def forward(self, a, r):
        """
        Propagate relevance scores through the DenseLayer.

        Args:
            a (torch.Tensor): Input activations.
            r (torch.Tensor): Relevance scores from the upper layers.

        Returns:
            torch.Tensor: Relevance scores for the input.
        """
        with torch.no_grad():
        ...
        # Split relevance into two parts
        r_input, r_new_output = torch.split(r, [a.size(1), activations[-1].size(1)], dim=1)

        activations.pop()  # ignore output of this bottleneck block
        activations = [a.data.requires_grad_(True) for a in activations]

        # Backward pass for the new output
        r_out = r_new_output
        for layer in reversed(self.layers):
            ...
            if isinstance(layer, nn.Conv2d):
                r_out = RelevancePropagationConv2d(layer, eps=self.eps, top_k=self.top_k)(activation, r_out)
            elif isinstance(layer, nn.BatchNorm2d):
                r_out = RelevancePropagationBatchNorm2d(layer, top_k=self.top_k)(activation, r_out)
            elif isinstance(layer, nn.ReLU):
                r_out = RelevancePropagationReLU(layer, top_k=self.top_k)(activation, r_out)
            else:
                raise NotImplementedError

        # Combine the relevance scores for the input
        return r_input + r_out


class RelevancePropagationTransitionBlock(nn.Module):
    def __init__(self, layer, eps=1.0e-12, top_k=0.0):
        """
        Layer-wise relevance propagation for _Transition in DenseNet.
        Attributes:
            layer: The _Transition to be handled.
            eps: Small value for numerical stability.
            top_k: Optional filter to keep top_k% of relevance scores.
        """
        super().__init__()
    ...
    def forward(self, a, r):
        """
        Propagate relevance scores through the Transition layer.
        ...
        for layer in reversed(self.layers):
            ...
            if isinstance(layer, nn.Conv2d):
                ...
                r_out = torch.nn.functional.interpolate(
                            r_out, scale_factor=(2, 2), mode="nearest"
                        ) / (2 ** 2)
            else:
                raise NotImplementedError
