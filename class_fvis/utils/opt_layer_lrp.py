...
from utils.lrp import LRPModel
import copy
...
def obtain_real_imgs(channel, num_real_img, layer, arch):
    """
    Loads and preprocesses real images for a specific channel, layer, and architecture.

    Args:
        channel (int): The channel index for which images are to be obtained.
        num_real_img (int): The number of real images to load.
        layer (str): The layer name for which images are being processed.
        arch (str): The architecture name (e.g., ResNet, DenseNet).
    ...
    main_dir = f'{arch}/neuron_{layer}/{channel}/top50/'
    txt_file = main_dir + 'files.txt'
